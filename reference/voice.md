...
* `session` (object | null) — Optional initial session configuration to bind to the client secret. This JSON value is stored alongside the secret and applied when the WebSocket connection opens.
  ...
  * `model` ("grok-voice-fast-1.0" | "grok-voice-think-fast-1.0") — Model to use for the session. For the best experience, use grok-voice-think-fast-1.0.
...
* `model` (string, optional, default: grok-voice-fast-1.0) — Model to use for the session. For the best experience, use grok-voice-think-fast-1.0
